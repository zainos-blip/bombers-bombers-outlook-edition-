...
def banner():
    print()
    print(bcolors.YELLOW + '                ~~~~~ Email-Bomber v1.0 ~~~~~')
    print(bcolors.YELLOW + '                ~~~~~ made with codes ~~~~~' + "\n")
    print(bcolors.RED + '''                              / \  //\\
                |\___/|      /   \//  .\\
                /O  O  \__  /    //  | \ \\
               /     /  \/_/    //   |  \  \\
...
class Email_Bomber:
    ...
    def __init__(self):
        try:
            ...
            if int(self.mode) > int(4) or int(self.mode) < int(1):
                print("Error: Invalid option. Good - bye")
                sys.exit(1)
    ...
    def bomb(self):
        try:
            ...
            if self.mode == int(1):
                ...
                self.amount = int(input(bcolors.GREEN + "Choose a CUSTOM input: "))
            
            print(bcolors.RED + f'\n~~~~~~ You have selected a BOMB mode: {self.mode} mode number and {self.amount} emails ~~~~~~~')
    ...
    def send(self):
        try:
            ...
            unique_identifier = str(self.count)
            subject_with_identifier = f"{self.subject} - {unique_identifier}"

            self.msg = '''From: %s\nTo: %s\nSubject: %s\n\n%s\n
            ''' % (self.fromAddr, self.target, f"{subject_with_identifier} - {timestamp}", self.message)
